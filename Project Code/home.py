...
def register_main():
    ...
    if r>0:
        ...
        for i in d:
            ...
            t1=tuple(l)
        if len(t1)<=1:
            r1=cur.execute("SELECT * FROM event where id!="+str(i[2])+" and uid!='"+str(id1)+"'")
            if r1>0:
                d1=cur.fetchall()
                return render_template("register_main.html",res=d1)
        else:
            r1=cur.execute("SELECT * FROM event where id not in "+str(t1)+" and uid!='"+str(id1)+"'")
            if r1>0:
                d1=cur.fetchall()
                return render_template("register_main.html",res=d1)
    else:
        r2=cur.execute("SELECT * FROM event where uid!='"+str(id1)+"'")
        d1=cur.fetchall()
        return render_template("register_main.html",res=d1)
